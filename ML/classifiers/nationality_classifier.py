"""
Nationality Classifier Module
Classifies whether a user is Russian/Ukrainian by nationality (not just living there).
A person can be Russian even if not living in Russia.
"""
...
class NationalityClassifier:
    """
    Classifier for determining if a user is Russian/Ukrainian by nationality.
    
    Note: This is different from LocalClassifier - a person can be Russian
    even if they don't live in Russia (e.g., Russian diaspora abroad).
    ...
    # Label column mapping by country
    LABEL_COLUMNS = {
        'Russia': 'russian:judge',
        'Ukraine': 'ukraine:judge'
    }
    ...
    def __init__(
        ...
        """
        Initialize the NationalityClassifier.
        ...
        Args:
            ...
            tfidf_columns: List of text columns to use with TF-IDF vectorization (e.g., ['bio'])
            label_column: The column name containing the labels (default: based on country)
        ...
        self.label_column = label_column if label_column else self.LABEL_COLUMNS[country]
        self.random_state = random_state
        self.results: List[ExperimentResult] = []
        self.iteration_counter = 0
        
        print(f"Nationality Classifier for {country}")
    ...
    CLASSIFIER_TYPE = 'nationality'
    
    def _load_data(self) -> None:
        """Load the data from the Excel file."""
        # Navigate from ML/classifiers/ to project root
        project_dir = Path(__file__).resolve().parent.parent.parent
        data_path = project_dir / "lib" / "Outputs" / self.country / f"{self.country}_nationality.xlsx"
    ...
    def _normalize_label(self, value) -> str:
        ...
        if isinstance(value, str):
            ...
            if value_lower in ('true', 't', 'yes', 'y', '1', 'russian', 'ukrainian'):
                return 'true'
            elif value_lower in ('false', 'f', 'no', 'n', '0', 'not russian', 'not ukrainian'):
                return 'false'
    ...
    def run_all_experiments(
        ...
        print(f"\n{'#'*60}")
        print(f"Starting {total_experiments} NATIONALITY experiments for {self.country}")
        print(f"{'#'*60}")
...
def main():
    """Main function to run the nationality classifier experiments."""
    import argparse
    
    parser = argparse.ArgumentParser(description='Run Nationality Classifier experiments')
    parser.add_argument(
        '--country', 
        type=str, 
        choices=['Russia', 'Ukraine'], 
        required=True,
        help='Country to classify nationality for'
    )
    parser.add_argument(
        '--features',
        type=str,
        nargs='+',
        default=None,
        help='Feature columns to use (default: classified_country, bio_language, location_language)'
    )
    parser.add_argument(
        '--tfidf',
        type=str,
        nargs='+',
        default=None,
        help='Text columns to use with TF-IDF (e.g., bio)'
    )
    parser.add_argument(
        '--algorithms',
        type=str,
    ...
    classifier = NationalityClassifier(
